...
namespace net {
    namespace tcp {
        ...
        HARE_IMPL_DEFAULT(serve,
            ...
            serve::new_session_callback new_session_ {};
        )

        serve::serve(io::cycle* _cycle, std::string _name)
        ...
        auto serve::main_cycle() const -> io::cycle*
        {
            return d_ptr(impl_)->cycle_;
        }

        auto serve::is_running() const -> bool
        {
            return d_ptr(impl_)->started_;
        }

        void serve::set_new_session(new_session_callback _new_session)
        {
            d_ptr(impl_)->new_session_ = std::move(_new_session);
        }

        auto serve::add_acceptor(const acceptor::ptr& _acceptor) -> bool
        {
            ...
            d_ptr(impl_)->cycle_->run_in_cycle([&] {
                ...
                _acceptor->set_new_session(std::bind(&serve::new_session, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
        ...
        auto serve::exec(std::int32_t _thread_nbr) -> error
        {
        ...
        void serve::new_session(util_socket_t _fd, host_address& _address, const timestamp& _time, acceptor* _acceptor)
        {
            assert(d_ptr(impl_)->started_);
            d_ptr(impl_)->cycle_->assert_in_cycle_thread();

            auto next_item = d_ptr(impl_)->io_pool_->get_next();
            session::ptr psession { nullptr };
            ...
            psession.reset(new session(next_item->cycle.get(),
                std::move(local_addr),
            ...
            auto sfd = psession->fd();

            psession->set_destroy([=]() {
            ...
            next_item->cycle->run_in_cycle([=]() mutable {
                ...
                next_item->sessions.insert(std::make_pair(sfd, psession));
                d_ptr(impl_)->new_session_(psession, _time, std::static_pointer_cast<acceptor>(_acceptor->shared_from_this()));
                psession->connect_established();
            });
        }
