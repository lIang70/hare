...
namespace net {
    ...
    HARE_IMPL_DEFAULT(
        ...
        Ptr<TcpSession> session {};
        io::Event::Id retry_id { -1 };
    )
    ...
    auto TcpClient::MainCycle() const -> io::Cycle*
    {
        return IMPL->cycle;
    }

    auto TcpClient::IsRunning() const -> bool
    {
        return IMPL->started;
    }
    ...
    void TcpClient::ConnectTo(const HostAddress& _address, bool _retry, std::int32_t _retry_times, std::int64_t _delay)
    {
        if (Connected()) {
            HARE_INTERNAL_ERROR("the client is connected, please reconnect after disconnecting.");
            return;
        }
        ...
        MainCycle()->QueueInCycle([=] {
            ...
            auto ret = socket_op::Connect(fd, peer_addr->get_sockaddr(), socket_op::AddrLen(family));
            ...
            case EINTR:
            case EISCONN:
                IMPL->session.reset(new TcpSession(IMPL->cycle,
                    HostAddress::LocalAddress(fd),
            ...
            case ENETUNREACH:
                ...
                if (_retry && _retry_times > 0) {
                    IMPL->retry_id = IMPL->cycle->RunAfter([=]() mutable {
            ...
            case ENOTSOCK:
                socket_op::Close(fd);
                HARE_INTERNAL_ERROR("a connection error {} occurred while connecting to {}", saved_errno, peer_addr->ToIpPort());
                IMPL->retry_id = -1;
                break;

            default:
                socket_op::Close(fd);
                HARE_INTERNAL_ERROR("a unexpected error {} occurred while connecting to {}", saved_errno, peer_addr->ToIpPort());
                IMPL->retry_id = -1;
                break;
            }
        });
