    ...
    , reactor_(core::Reactor::createByType(reactor_type, this))
{
    ...
    wake_up_event_->setFlag(Event::EV_READ | Event::EV_PERSIST);
}
...
void Cycle::loop()
{
    ...
    while (!quit_) {
        ...
        for (auto& channel : active_events_) {
            current_active_event_ = channel;
...
void Cycle::printActiveEvents() const
{
    for (const auto& event : active_events_) {
        LOG_TRACE() << "event[" << event->fd() << "] debug info: " << event->d_->reventsToString() << ".";
    }
}
