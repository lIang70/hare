...
namespace net {
    namespace socket_op {
        ...
        {
            return ::connect(_fd, _addr, sizeof(struct sockaddr)) < 0 ? error(ERROR_SOCKET_CONNECT) : error();
        }
        ...
        auto close(util_socket_t _fd) -> error
        {
            ...
            return ret < 0 ? error(ERROR_SOCKET_CLOSED) : error();
        }
        ...
        auto from_ip_port(const char* target_ip, uint16_t port, struct sockaddr_in* _addr) -> error
        {
            ...
            return ::inet_pton(AF_INET, target_ip, &_addr->sin_addr) <= 0 ? error(ERROR_SOCKET_FROM_IP) : error();
        }
        ...
        auto from_ip_port(const char* target_ip, uint16_t port, struct sockaddr_in6* _addr) -> error
        {
            ...
            return ::inet_pton(AF_INET6, target_ip, &_addr->sin6_addr) <= 0 ? error(ERROR_SOCKET_FROM_IP) : error();
        }
