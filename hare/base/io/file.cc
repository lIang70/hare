...
namespace io {
    ...
    namespace file_inner {
        ...
        auto Open(std::FILE** _fp, const filename_t& _filename, const filename_t& _mode) -> bool
        {
#if defined(H_OS_WIN32) && HARE_WCHAR_FILENAME
            *_fp = ::_wfsopen(FilenameToStr(_filename).c_str(), FilenameToStr(_mode).c_str(), _SH_DENYWR);
#elif defined(H_OS_WIN32)
            *_fp = ::_fsopen(FilenameToStr(_filename).c_str(), FilenameToStr(_mode).c_str(), _SH_DENYWR);
#else // unix
            *_fp = ::fopen(FilenameToStr(_filename).c_str(), FilenameToStr(_mode).c_str());
#endif
            return *_fp != nullptr;
        }
