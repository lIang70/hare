...
auto timestamp::to_string() const -> std::string
{
    ...
    ::snprintf(buffer.data(), HARE_SMALL_FIXED_SIZE, "%" PRId64 ".%.06" PRId64, seconds, microseconds);
    return buffer.data();
}
...
auto timestamp::to_fmt(bool show_microseconds) const -> std::string
{
    ...
    if (show_microseconds) {
        ...
        ::snprintf(buffer.data(), buffer.size(), "%4d-%02d-%02d %02d:%02d:%02d.%06d",
            tm_time.tm_year + HARE_START_YEAR, tm_time.tm_mon + 1, tm_time.tm_mday,
            tm_time.tm_hour, tm_time.tm_min, tm_time.tm_sec,
            microseconds);
    } else {
        ::snprintf(buffer.data(), buffer.size(), "%4d-%02d-%02d %02d:%02d:%02d",
            tm_time.tm_year + HARE_START_YEAR, tm_time.tm_mon + 1, tm_time.tm_mday,
            tm_time.tm_hour, tm_time.tm_min, tm_time.tm_sec);
    }
