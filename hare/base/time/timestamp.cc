...
auto timestamp::to_string() const -> std::string
{
    ...
    auto microseconds = microseconds_since_epoch_ - seconds * static_cast<int64_t>(HARE_MICROSECONDS_PER_SECOND);
...
auto timestamp::to_fmt(bool show_microseconds) const -> std::string
{
    ...
    if (show_microseconds) {
        auto microseconds = static_cast<int32_t>(microseconds_since_epoch_ - seconds * static_cast<int64_t>(HARE_MICROSECONDS_PER_SECOND));
