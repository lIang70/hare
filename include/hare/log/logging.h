...
class HARE_API Logger : public ::hare::NonCopyable {
 ...
 public:
  HARE_INLINE
  void set_level(Level _level) { level_.store(_level); }
  ...
  HARE_INLINE
  void FlushOn(Level _level) { flush_level_.store(_level); }

  HARE_INLINE
  void set_timezone(const Timezone& _tz) { timezone_ = _tz; }

  HARE_INLINE
  auto name() const -> std::string { return name_; }

  HARE_INLINE
  auto backends() const -> BackendList { return backends_; }

  template <typename... Args>
  HARE_INLINE void Log(SourceLoc& _loc, Level _level,
                       fmt::format_string<Args...> _fmt, const Args&... _args) {
    if (!Check(_level)) {
      return;
    }
  ...
  template <typename... Args>
  HARE_INLINE void Trace(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                         const Args&... _args) {
    Log(_loc, LEVEL_TRACE, _fmt, _args...);
  }

  template <typename... Args>
  HARE_INLINE void Debug(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                         const Args&... _args) {
    Log(_loc, LEVEL_DEBUG, _fmt, _args...);
  }

  template <typename... Args>
  HARE_INLINE void Info(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                        const Args&... _args) {
    Log(_loc, LEVEL_INFO, _fmt, _args...);
  }

  template <typename... Args>
  HARE_INLINE void Warning(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                           const Args&... _args) {
    Log(_loc, LEVEL_WARNING, _fmt, _args...);
  }

  template <typename... Args>
  HARE_INLINE void Error(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                         const Args&... _args) {
    Log(_loc, LEVEL_ERROR, _fmt, _args...);
  }

  template <typename... Args>
  HARE_INLINE void Fatal(SourceLoc&& _loc, fmt::format_string<Args...> _fmt,
                         const Args&... _args) {
...
#define LOG_WARNING(logger, format, ...) \
  logger->Warning({__FILE__, __LINE__, __func__}, format, ##__VA_ARGS__)
