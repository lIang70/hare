...
class HARE_API ThreadPool final {
 ...
 public:
  ...
  ThreadPool(std::size_t _max_items, std::size_t _thr_n, TaskHandle _thr_task)
      : queue_(_max_items), threads_(_thr_n), handle_(std::move(_thr_task)) {
  ...
  HARE_INLINE
  auto ThreadSize() const -> std::size_t { return threads_.size(); }

  HARE_INLINE
  auto OverCounter() const -> std::size_t { return queue_.OverCounter(); }

  HARE_INLINE
  void ResetCounter() { queue_.ResetCounter(); }

  HARE_INLINE
  auto QueueSize() -> std::size_t { return queue_.Size(); }
