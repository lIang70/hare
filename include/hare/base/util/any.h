...
namespace util {
    ...
    class HARE_API Any {
        ...
        struct HARE_API Derived final : public Base {
            ...
            auto ObjTypeId() const noexcept -> const void* final { return IdForType<T>(); }

            auto Type() const noexcept -> const std::type_info& final { return typeid(T); }
        };
    ...
    public:
        ...
        Any(T&& value)
            : ptr_(new Derived<VT>(in_place, std::forward<T>(value)))
        {
        }
            ...
            hare::enable_if_t<
                hare::conjunction<std::is_copy_constructible<VT>,
                    std::is_constructible<VT, std::initializer_list<U>&,
                        Args...>>::value>* = nullptr>
        explicit Any(hare::detail::in_place_type_t<T> /*tag*/, std::initializer_list<U> ilist,
            Args&&... args)
        ...
        template <
            ...
            hare::enable_if_t<std::is_copy_constructible<VT>::value && std::is_constructible<VT, Args...>::value>* = nullptr>
        auto emplace(Args&&... args) HARE_ATTRIBUTE_LIFETIME_BOUND->VT&
        {
            Reset(); // NOTE: reset() is required here even in the world of exceptions.
            Derived<VT>* const object_ptr = new Derived<VT>(in_place, std::forward<Args>(args)...);
    ...
    auto AnyCast(const Any* operand) noexcept -> const T*
    {
        ...
        return operand && operand->GetObjTypeId() == Any::IdForType<U>()
            ? std::addressof(
                static_cast<const Any::Derived<U>*>(operand->ptr_.get())->value)
            : nullptr;
    }
